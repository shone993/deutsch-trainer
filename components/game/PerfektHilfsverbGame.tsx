  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full max-w-lg mx-auto">
      ...
      <div className="w-full bg-purple-600 text-white rounded-2xl px-5 py-4 text-center">
        <p className="text-xs text-purple-200 uppercase tracking-wide mb-1">Perfekt — pomoćni glagol</p>
      ...
      <p className="text-sm text-gray-500 self-start">
        🤔 Koji pomoćni glagol se koristi u Perfektu?
      </p>
      ...
      {selected && (
        <div className={`w-full rounded-xl p-4 text-center font-semibold text-lg ${
          ...
          {selected === correct
            ? `🎉 Tačno! ${question.infinitiv} koristi "${correct}"`
            : `❌ Tačan odgovor: ${correct}`}
        </div>
      )}
    </div>
  )
