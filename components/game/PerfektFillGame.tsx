...
export function PerfektFillGame({ question, onAnswer, questionNumber, totalQuestions }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitted(true)
    setTimeout(() => {
      onAnswer({ questionId: question.id, userAnswer: inputs.join(' | '), isCorrect: allCorrect, timeTakenMs, pointsEarned: calculatePoints(allCorrect, timeTakenMs) })
    }, 1500)
  }
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full max-w-lg mx-auto">
      ...
      <div className="w-full bg-purple-600 text-white rounded-2xl px-5 py-4 text-center">
        <p className="text-xs text-purple-200 uppercase tracking-wide mb-1">{g.perfektFillHeader}</p>
        <p className="text-2xl font-bold">{question.infinitiv}</p>
      </div>

      <p className="text-sm text-gray-500 self-start">{g.perfektFillInstruction}</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 w-full">
        <div className="text-lg text-gray-900 leading-loose flex flex-wrap items-center gap-1">
          {parts.map((part, i) => {
            ...
            const val = submitted
              ...
              : (inputs[idx] || (idx === 0 ? 'aux' : 'Partizip II'))
            return (
        ...
        <div className="flex items-center gap-2 mt-3 text-xs text-gray-400">
          <span className="bg-purple-100 text-purple-600 px-2 py-0.5 rounded">{g.perfektAuxBadge}</span>
          <span className="bg-purple-100 text-purple-600 px-2 py-0.5 rounded">{g.perfektPartizipBadge}</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-2">
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs text-gray-500 mb-1 block">{g.perfektAuxLabel}</label>
            <input ref={inputRefs[0]} type="text" value={inputs[0]}
              onChange={(e) => setInputs([e.target.value, inputs[1]])} disabled={submitted}
              ...
              placeholder="habe / bin / hat ..." autoCorrect="off" autoCapitalize="none" spellCheck={false} />
          </div>
          <div className="flex-1">
            <label className="text-xs text-gray-500 mb-1 block">{g.perfektPartizipLabel2}</label>
            <input ref={inputRefs[1]} type="text" value={inputs[1]}
              onChange={(e) => setInputs([inputs[0], e.target.value])} disabled={submitted}
              className="w-full border-2 border-gray-300 focus:border-purple-500 rounded-xl px-3 py-2.5 text-base outline-none transition disabled:opacity-60"
              placeholder="gelernt / gegangen ..." autoCorrect="off" autoCapitalize="none" spellCheck={false} />
          </div>
        </div>
      ...
      {submitted && (
        <div className={`w-full rounded-xl p-4 text-center font-semibold ${
          ...
          {correct.every(Boolean) ? g.correct : (
            <div>
              <div>{g.correctAnswers}</div>
              {blanks.map((b, i) => (
                <div key={i} className="text-sm mt-1">
                  {i === 0 ? g.perfektAuxName : g.perfektPartizipName}: <strong>{b.answer}</strong>
                  {!correct[i] && <span className="text-red-400"> ({g.youWrote} {inputs[i]})</span>}
                </div>
              ))}
            </div>
          )}
