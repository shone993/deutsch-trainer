...
export function AudioGame({ question, onAnswer, questionNumber, totalQuestions }: Props) {
  ...
  // Automatski pusti audio kad se učita novo pitanje
  useEffect(() => {
    ...
    <div className="flex flex-col items-center gap-5 w-full max-w-lg mx-auto">
        ...
        <button
          ...
          className={`w-28 h-28 rounded-full flex items-center justify-center text-6xl transition-all select-none ${
            isPlaying
              ...
              : 'bg-white/20 hover:bg-white/30 hover:scale-105 shadow-lg'
          }`}
        >
          <span className={isPlaying ? 'animate-pulse' : ''}>🔊</span>
        </button>
        <p className="text-sky-200 text-sm">
          {isPlaying ? 'Svira…' : 'Klikni da ponovo čuješ'}
        </p>
      </div>

      {/* Uputstvo */}
      <p className="text-sm font-medium text-gray-600 self-start">
        Koji infinitiv si čuo/la?
      </p>
      ...
      <div className="w-full grid grid-cols-2 gap-3">
        {options.map((opt) => {
          ...
          if (selected === opt && !submitted)
            cls = 'border-2 border-sky-500 bg-sky-50 text-sky-800'
          if (submitted && isCorrectOpt)
            cls = 'border-2 border-green-500 bg-green-50 text-green-800'
          if (submitted && selected === opt && !isCorrectOpt)
      ...
      {submitted && (
        <div className={`w-full rounded-xl p-4 text-center font-semibold text-lg ${
          ...
          {isCorrect
            ? `🎉 Tačno! Čuo/la si: „${audioText}"`
            : `❌ Tačno: ${question.correctAnswers[0]} — čulo se: „${audioText}"`}
        </div>
      )}
    </div>
  )
