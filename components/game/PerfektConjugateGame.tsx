...
const PERSON_DE: Record<string, string> = {
...
export function PerfektConjugateGame({ question, onAnswer, questionNumber, totalQuestions }: Props) {
  ...
  const personNative = PERSON_DE[pronoun] ? g.persons[
    pronoun === 'er/sie/es' ? 'er' : pronoun === 'sie/Sie' ? 'sie' : pronoun as keyof typeof g.persons
  ] : ''
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full max-w-lg mx-auto">
      ...
      <div className="w-full bg-purple-600 text-white rounded-2xl px-5 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-purple-200 uppercase tracking-wide mb-0.5">{g.perfektConjugateVerbLabel}</p>
          <p className="text-2xl font-bold">{question.infinitiv}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-purple-200 uppercase tracking-wide mb-0.5">{g.personLabel}</p>
          <p className="text-xl font-bold">{pronoun}</p>
          {personNative && <p className="text-sm text-purple-200">({personNative})</p>}
        </div>
      </div>

      <p className="text-sm text-gray-500 self-start">{g.perfektConjugateInstruction}</p>

      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 w-full">
        <div className="flex items-center justify-center gap-3 text-xl flex-wrap">
          <span className="bg-purple-100 text-purple-700 font-bold px-3 py-1.5 rounded-lg">{pronoun}</span>
        ...
        {!submitted && (
          <p className="text-center text-gray-400 text-xs mt-3">npr. <em>{aux} {partizip}</em></p>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full flex gap-2">
        <input ref={inputRef} type="text" value={input}
          ...
          placeholder={`${pronoun} ${g.perfektConjugatePlaceholder}`}
